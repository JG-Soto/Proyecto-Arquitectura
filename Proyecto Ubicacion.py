...
class MonitoreoSistemaApp:
    ...
    def enviar_correo_gmail(self,asunto, cuerpo, destinatario):
        ...
        gmail_password = 'lqus vgvb lzhq xacq'
    ...
    def monitorear_cpu(self):
        ...
        while not self.detener_ejecucion:
            ...
            if uso > self.umbral:
                ...
                self.ventana.after(0, lambda: self.mostrar_ventana(mensaje))
            else:
                self.ventana.after(0, self.ocultar_ventana)
...
if __name__ == "__main__":
    ...
    hilo_cpu = Thread(target=app_cpu.monitorear_cpu, daemon=True)
    hilo_memoria = Thread(target=app_memoria.monitorear_memoria, daemon=True)
